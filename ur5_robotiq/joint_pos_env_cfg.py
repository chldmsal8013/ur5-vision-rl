...
from isaaclab_tasks.manager_based.manipulation.lift.config.ur5_robotiq.commands import TargetColorCommandCfg
...
class UR5RobotiqCubeLiftEnvCfg_State(UR5RobotiqCubeLiftEnvCfg):
    def __post_init__(self):
        ...
        # --- 대조군 실험: 복잡한 shaped grasp reward를 끄고, Franka 스타일 단순 binary로 교체 ---
        self.rewards.object_is_lifted_bonus.weight = 0.0
        self.rewards.grasp_contact_bonus.weight = 0.0
        self.rewards.simple_lift_bonus = RewTerm(
            func=mdp.object_is_lifted_target_aware,
            weight=15.0,
        ...
        self.rewards.reaching_object = RewTerm(
            func=mdp.object_ee_distance_target_aware,
            weight=0.8,
            params={"std": 0.3},
        )
        self.rewards.home_pose_after_lift = RewTerm(
            func=mdp.home_pose_after_lift,
            weight=3.0,
            params={
                "minimal_height": 0.05,
                "std": 0.5,
